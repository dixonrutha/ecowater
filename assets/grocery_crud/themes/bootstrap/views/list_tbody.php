        ...
        <td <?php if ($unset_delete) { ?> style="border-left: none;"<?php } ?>
            <?php if ($buttons_counter === 0) {?>class="hidden"<?php }?>>
                <div class="only-desktops"  style="white-space: nowrap">
                    <?php if(!$unset_edit){?>
                        <a class="btn btn-xs btn-outline-warning" href="<?php echo $row->edit_url?>" data-toggle="tooltip" data-placement="top" title="Edit"><i class="fa fa-edit"></i></a>
                    <?php } ?>
                    <?php if (!empty($row->action_urls) || !$unset_read || !$unset_delete || !$unset_clone) { ?>

                        <?php if ($show_more_button) { ?>
                            <div class="btn-group">
                                <button type="button" class="btn btn-xs btn-default btn-outline-dark gc-bootstrap-dropdown dropdown-toggle">
                            ...
                            <?php } else {
                                ...
                                if (!$unset_read) { ?>
                                    <a href="<?php echo $row->read_url?>" class="btn btn-xs btn-default btn-outline-dark">
                                        <i class="el el-eye-open"></i> <?php echo $this->l('list_view')?>
                                    </a>
                                <?php } ?>

                                <?php if (!$unset_clone) { ?>
                                    <a href="<?php echo $row->clone_url?>" class="btn btn-xs btn-default btn-outline-dark">
                                        <i class="el el-fork"></i> <?php echo $clone_string; ?>
                                    </a>
                                <?php } ?>

                                <?php if (!$unset_delete) { ?>
                                    <a data-target="<?php echo $row->delete_url?>" href="javascript:void(0)" title="<?php echo $this->l('list_delete')?>" class="delete-row btn btn-xs btn-default btn-outline-dark">
                                        <i class="fa fa-trash text-danger"></i>
                                        <span class="text-danger"><?php echo $this->l('list_delete')?></span>
                                    </a>
                                <?php } ?>
                ...
                <div class="only-mobiles">
                    ...
                    <div class="btn-group">
                        <button type="button" class="btn btn-xs btn-secondary gc-bootstrap-dropdown dropdown-toggle">
